...
can be, other that it must be different for every race. It does not have to list them sequentially, it can be random
digits so long as they are unique.
...
`skaterColours`: An array of two-large subarrays determining which colour will be linked with a given club code. For
...
correct CORS headers, or the request will fail. More information relating to CORS can be
